...
import { UI_DEFAULTS, UI_OPTIONS, validUiSettings } from '../public/ui-settings.js';
...
export function createUiSettingsStore(filename) {
  ...
  async function read() {
    ...
    try {
      ...
      if (data?.version !== 1 || !validUiSettings(data.settings)) throw new Error('Invalid settings');
  ...
  return {
    ...
    replace: settings => serialized(() => write(structuredClone(settings))),
    change: body => serialized(async () => {
      if (!body || typeof body !== 'object' || Array.isArray(body) || Object.keys(body).length !== 2
        || !Object.hasOwn(UI_OPTIONS, body.key) || !UI_OPTIONS[body.key].includes(body.value)) {
