...
const descriptionLanguages = new Set(['english', 'russian']);
const validBytes = n => Number.isSafeInteger(n) && n > 0 && n <= 1024 ** 5;

export function cleanStoreDescription(value) {
  if (typeof value !== 'string' || !value || value.length > 20000) return null;
  const text = value.replace(/<(script|style)\b[^>]*>[\s\S]*?<\/\1>/gi, ' ')
    .replace(/<[^>]*>/g, ' ')
    .replace(/&nbsp;|&#160;|&#x0*a0;/gi, ' ')
...
export function createOnlineSizeService({ filename, send = globalThis.fetch, now = Date.now, interval = 2000, wait = ms => new Promise(resolve => setTimeout(resolve, ms)) } = {}) {
    ...
    if (!loaded) loaded = (async () => {
      ...
      try {
        ...
        for (const [key, item] of entries) {
          const size = /^(windows|linux|macos):([1-9]\d{0,9})$/.exec(key);
          const description = /^description:(english|russian):([1-9]\d{0,9})$/.exec(key);
          const commonValid = Number.isSafeInteger(item?.checkedAt) && item.checkedAt >= 0 && item.checkedAt <= now() + 60000;
          const valueValid = size ? validId(size[2]) && (item.bytes === null || validBytes(item.bytes))
            : description ? validId(description[2]) && (item.text === null || (typeof item.text === 'string' && item.text.length > 0 && item.text.length <= 500)) : false;
          if (!commonValid || !valueValid) throw new Error('Invalid cached Steam data');
        }
  ...
  const descriptionResult = (id, language, item, status, cached = false) => ({
    status, cached, cacheSaved: writable,
  ...
  return {
    ...
    async lookupDescription(id, language = 'english') {
      if (!validId(id) || !descriptionLanguages.has(language)) throw Object.assign(new Error('Некорректная игра или язык описания.'), { status: 400 });
      await load();
      const key = `description:${language}:${id}`, old = cache.get(key);
      if (old && now() - old.checkedAt < (old.text ? SIZE_CACHE_TTL : SIZE_MISS_TTL)) return descriptionResult(id, language, old, old.text ? 'ready' : 'not-found', true);
      if (pending.has(key)) return pending.get(key);
      if (now() < blockedUntil || now() < (retryAt.get(key) ?? 0)) return descriptionResult(id, language, old, 'offline', !!old);
      if (pending.size >= 6) return descriptionResult(id, language, old, 'busy', !!old);
      const task = (async () => {
        try {
          const turn = startQueue.then(async () => { await wait(Math.max(0, lastStart + interval - now())); lastStart = now(); });
          startQueue = turn.catch(() => {}); await turn;
          if (now() < blockedUntil) return descriptionResult(id, language, old, 'offline', !!old);
          const url = `https://store.steampowered.com/api/appdetails?appids=${id}&l=${language}&filters=basic`;
          const response = await send(url, { signal: AbortSignal.timeout(8000), redirect: 'error', credentials: 'omit', headers: { Accept: 'application/json' } });
          const json = await boundedJson(response);
          const entry = json?.[id];
          if (typeof entry?.success !== 'boolean') throw new Error('Malformed Steam response');
          if (entry.success && (!entry.data || String(entry.data.steam_appid) !== id)) throw new Error('Wrong Steam AppID');
          const text = entry.success ? cleanStoreDescription(entry.data.short_description) : null;
          if (!text && old?.text) { retryAt.set(key, now() + SIZE_MISS_TTL); return descriptionResult(id, language, old, 'not-found', true); }
          const item = { text, checkedAt: now() };
          cache.delete(key); cache.set(key, item);
          retryAt.delete(key); await persist();
          return descriptionResult(id, language, item, text ? 'ready' : 'not-found');
        } catch (error) {
          retryAt.set(key, now() + 5 * 60000);
          if ([403, 429].includes(error.upstreamStatus)) blockedUntil = now() + 30 * 60000;
          else blockedUntil = Math.max(blockedUntil, now() + 30000);
          return descriptionResult(id, language, old, 'offline', !!old);
        } finally { pending.delete(key); }
      })();
      pending.set(key, task);
      return task;
