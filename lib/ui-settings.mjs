...
export function createUiSettingsStore(filename) {
  ...
  async function read(defaultLanguage = UI_DEFAULTS.language, persistMissing = false) {
    let file;
    try {
      ...
      if (!file && error.code === 'ENOENT') {
        const settings = { ...UI_DEFAULTS, language: UI_OPTIONS.language.includes(defaultLanguage) ? defaultLanguage : UI_DEFAULTS.language };
        return persistMissing ? write(settings) : settings;
      }
  ...
  return {
    ...
    initialize: defaultLanguage => serialized(() => read(defaultLanguage, true)),
