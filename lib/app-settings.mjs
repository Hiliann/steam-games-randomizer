...
import { APP_DEFAULTS, normalizeAppSettings, validAppSettings } from '../public/app-settings.js';
...
export function createAppSettingsStore(filename) {
  ...
  async function read() {
    ...
    try {
      ...
      const settings = data?.version === 1 || data?.version === 2 ? normalizeAppSettings(data.settings) : null;
  ...
  return {
    ...
    replace: settings => serialized(async () => {
      if (!validAppSettings(settings)) throw Object.assign(new Error('Некорректные настройки программы.'), { status: 400 });
      return write(structuredClone(settings));
    }),
