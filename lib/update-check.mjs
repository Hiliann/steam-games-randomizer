...
function safeChecksumUrl(value, version) {
  if (typeof value !== 'string') return null;
  try {
    const url = new URL(value);
    const expected = `/Hiliann/steam-games-randomizer/releases/download/v${version}/PlayNext-${version}-win-x64.zip.sha256`;
    return url.protocol === 'https:' && url.hostname === 'github.com' && url.pathname === expected ? url.href : null;
  } catch { return null; }
}
...
export function createUpdateService({ currentVersion, request = fetch, clock = Date.now, cacheMilliseconds = 6 * 60 * 60 * 1000 } = {}) {
  ...
  const check = async ({ force = false } = {}) => {
    ...
    try {
      ...
      const checksumAsset = release.assets.find(item => item?.name === `${expectedName}.sha256`);
      result = {
        status: 'ready',
        currentVersion,
        latestVersion,
        ...
        checksumUrl: safeChecksumUrl(checksumAsset?.browser_download_url, latestVersion),
